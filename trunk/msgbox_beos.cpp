...
SDL_bool msgbox( struct machine *oric, int type, char *msg )
{
  ...
  {
    case MSGBOX_YES_NO:
      return ((new BAlert("Oriculator Request", msg, "Yes", "No"))->Go() == 0) ? SDL_TRUE : SDL_FALSE;
//      return (MessageBoxA( hwnd, msg, "Oriculator Request", MB_YESNO ) == IDYES);

    case MSGBOX_OK_CANCEL:
      return ((new BAlert("Oriculator Request", msg, "Ok", "Cancel"))->Go() == 0) ? SDL_TRUE : SDL_FALSE;
//      return (MessageBoxA( hwnd, msg, "Oriculator Request", MB_OKCANCEL ) == IDOK);
    
    case MSGBOX_OK:
      (new BAlert("Oriculator Request", msg, "Ok"))->Go();
      return SDL_TRUE;
  }
