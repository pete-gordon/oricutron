...
static inline char diskimage_check(struct diskimage* dimg, Uint32 offset)
{
  if(!dimg) return 0;
  if(!dimg->rawimage) return 0;
...
static Uint32 diskimage_get_rawint(struct diskimage* dimg, Uint32 offset)
{
  Uint32 val = 0;
  if(diskimage_check(dimg,offset))
  {
    val |= dimg->rawimage[offset+3]<<24;
    val |= dimg->rawimage[offset+2]<<16;
    val |= dimg->rawimage[offset+1]<<8;
    val |= dimg->rawimage[offset];
  }
  return val;
}

static void diskimage_set_rawint(struct diskimage* dimg, Uint32 offset, Uint32 val)
{
  if(diskimage_check(dimg, offset))
  {
    dimg->rawimage[offset] = val & 0xff;
    val >>= 8;
    dimg->rawimage[offset+1] = val & 0xff;
    val >>= 8;
    dimg->rawimage[offset+2] = val & 0xff;
    val >>= 8;
    dimg->rawimage[offset+3] = val & 0xff;
  }
}
...
static void diskimage_set_modified( struct diskimage* dimg, uint8_t side, uint8_t track )
{
  if(dimg)
  {
    dimg->modified = SDL_TRUE;
    dimg->modified_time = 0;
    side++;
    if(dimg->numsides_out < side)
      dimg->numsides_out = side;
    track++;
    if(dimg->numtracks_out < track)
      dimg->numtracks_out = track;
  }
}
...
SDL_bool diskimage_save( struct machine *oric, char *fname, int drive )
{
  ...
  else
  {
    ...
    // Dump it to disk
    if(dimg->numsides_in < dimg->numsides_out)
      dimg->numsides_in = dimg->numsides_out;
    if(dimg->numtracks_in < dimg->numtracks_out)
      dimg->numtracks_in = dimg->numtracks_out;

    diskimage_set_rawint(dimg, 8, oric->wddisk.disk[drive]->numsides_in);
    diskimage_set_rawint(dimg, 12, oric->wddisk.disk[drive]->numtracks_in);

    sidesize = oric->wddisk.disk[drive]->numtracks_in*6400;
...
SDL_bool diskimage_load( struct machine *oric, char *fname, int drive )
{
  ...
  else
  {
    ...
    // Get some basic image info
    oric->wddisk.disk[drive]->drivenum  = drive;
    oric->wddisk.disk[drive]->numsides_in  = diskimage_get_rawint(oric->wddisk.disk[drive], 8);
    oric->wddisk.disk[drive]->numtracks_in = diskimage_get_rawint(oric->wddisk.disk[drive], 12);
    oric->wddisk.disk[drive]->geometry  = diskimage_get_rawint(oric->wddisk.disk[drive], 16);
    ...
    if((oric->wddisk.disk[drive]->numsides_in < 1) ||
    ...
    oric->wddisk.disk[drive]->numtracks = 128;

    if(1 < oric->wddisk.disk[drive]->numsides_in)
    {
      if(oric->wddisk.disk[drive]->numtracks_in < 128)
      {
        // Reallocate side 2
        int size = oric->wddisk.disk[drive]->numtracks_in*6400;
        uint8_t* src = oric->wddisk.disk[drive]->rawimage + 256+size;
